 ...
 List<Poster> postersList = [];
  ...
  Widget getBackgroundImage() {
    ...
    if(posty < 0 && slides.length>0 )
      return ImageFade(image:NetworkImage(slides[side_current].image),fit: BoxFit.cover);
    if(posty == 0 && channels.length>0)
      return ImageFade(image:NetworkImage(channels[postx].image),fit: BoxFit.cover);
    if(posty > 0 && genres.length>0)
      return ImageFade(image:NetworkImage(genres[posty].posters[postx].cover),fit: BoxFit.cover);
